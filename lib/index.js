...
// * gaps are excluded
stat.prototype.icCalc = function icCalc(opts) {
...
stat.prototype.conservation = function conservation(alphabetSize, opts) {
  var ic;
  if(opts){
    ic = this.icCalc(opts);
  } else{
...
stat.prototype.conservResidue = function conservation(input) {
  ...
  if (input !== undefined && input.scaled) {
    ic = this.scale(this.conservation(alphabetSize, {ignoredChars: true}));
    //console.log(this.icCalc({ignoredChars: true}));
    //console.log(this.icCalc({ignoredChars: false}));
  } else {
    ic = this.conservation(alphabetSize, {ignoredChars: true});
  }
  var f = this.frequency();
  var gaps = this.gaps();
  gaps = _.map(gaps, function(el){
    return 1 - el;
  });
  var keys;
  var conserv = _.map(f, function(el, i) {
    ...
    _.each(keys, function(key) {
      var fact = 1;
      if(Object.keys(f[i]).length === 1){
        fact = gaps[i];
      }
      // FIX ME
      obj[key] = el[key] * ic[i] * fact;
    });
    return obj;
  });
  return conserv;
